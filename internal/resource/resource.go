// Package resource 统一定位随仓库分发的运行时资源（skills/、.venv/ 等）。
//
// 定位优先级（skills_dir 显式配置 > 自动探测）：
//  1. config.yaml 的 skills_dir（SetSkillsDir 注入；绝对路径直接生效）
//  2. $YTB2BILI_PROJECT_DIR（显式指定，最可靠）
//  3. 可执行文件所在目录（若其下存在 skills/）
//  4. 当前工作目录（若其下存在 skills/）
//  5. 源码位置回溯（go run / 测试场景：internal/resource/ → 项目根）
//
...
// configuredSkillsDir 由 config.yaml skills_dir 显式指定（空 = 自动探测）。
// 由 cli 包在配置加载完成后注入一次，运行期间只读。
var configuredSkillsDir string

// SetSkillsDir 注入 config.yaml 的 skills_dir 配置。
// 空串恢复自动探测；相对路径相对项目根解析。
...
// SkillsDir 返回 skills 目录绝对路径。
func SkillsDir() string {
	if d := configuredSkillsDir; d != "" {
		if filepath.IsAbs(d) {
			return d
		}
...
func SkillScript(rel string) string {
	return filepath.Join(SkillsDir(), rel)
}
